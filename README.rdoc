...
== Usage
  ...
  jazor [options] [source] [expression(s)]
...
a *real* HTTP client like {cURL}[http://curl.haxx.se/] and simply pipe the
output to Jazor (see examples).

=== Expressions

The *expression(s)* argument is pure Ruby code that is simply eval'ed within
the context of your JSON object. After Jazor parses your JSON input into native
Ruby data types (Hash, Array, etc.), you can use expressions to slice and dice
the data any way you want. The results will be "pretty printed" to STDOUT.
...
with a non-zero return code. This could be helpful when using Jazor in shell
scripts.
